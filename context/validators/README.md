...
- proves: required files exist, scripts are executable, install/test hooks exist, the bundled Go wrapper is present, the config template exists, the local commit-message rule exists, unsafe C functions are absent, and the logger still uses non-blocking queue admission.
